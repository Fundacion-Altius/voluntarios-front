...
export default function MensajesPage() {
  ...
  if (loading) return <div><PageHeader title="Mensajes" /><LoadingSkeleton rows={3} /></div>;
  if (error) return <div><PageHeader title="Mensajes" /><ErrorState message={error} /></div>;
    ...
    <div className="space-y-6">
              ...
              <div className="space-y-4 pt-2">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input value={searchQuery} onChange={(e) => handleSearch(e.target.value)} placeholder="Escribe un nombre..." className="pl-9" />
                  </div>
                  ...
                  {selectedUser && (
                    <div className="flex items-center gap-2 rounded-md bg-primary/5 px-3 py-2 text-sm">
                      <span>Participante: {selectedUser.name || selectedUser.email}</span>
                      <button className="ml-auto text-muted-foreground hover:text-foreground" onClick={() => setSelectedUser(null)}>x</button>
                    </div>
                  )}
                </div>
                <Button onClick={createConversation} className="w-full" disabled={!newConvTitle.trim()}>Crear</Button>
              </div>
            </DialogContent>
          </Dialog>
        }
      ...
      {conversations.length === 0 ? (
        <EmptyState
          ...
          description="Crea una para empezar a chatear con tu equipo"
        />
      ) : (
        <div className="space-y-2">
