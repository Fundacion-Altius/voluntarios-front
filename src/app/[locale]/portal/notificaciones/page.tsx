...
export default function NotificacionesPage() {
  ...
  const t = useTranslations('portal.notificaciones');
        ...
        <>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 pt-4">
              <Button variant="outline" size="sm" disabled={page <= 1} onClick={() => setPage((p) => p - 1)}>{t('anterior')}</Button>
              <span className="text-sm text-muted-foreground">{page} / {totalPages}</span>
              <Button variant="outline" size="sm" disabled={page >= totalPages} onClick={() => setPage((p) => p + 1)}>{t('siguiente')}</Button>
            </div>
          )}
        </>
      ) : (
        <EmptyState title={t('sinNotificaciones')} />
      )}
    </div>
  );
}
