...
export default function NoticiaDetailPage() {
  ...
  const t = useTranslations('portal.noticia');
  ...
  if (!post) {
    return (
      <div className="space-y-4">
        <p className="text-muted-foreground">{t('noEncontrada')}</p>
        <Link href="/portal/noticias"><Button variant="outline"><ArrowLeft className="mr-1 size-4" /> {t('volver')}</Button></Link>
      </div>
    );
  }
  ...
  return (
    <article className="space-y-6">
      <Link href="/portal/noticias">
        <Button variant="ghost" size="sm"><ArrowLeft className="mr-1 size-4" /> {t('volverNoticias')}</Button>
      </Link>
      {post.image_url && (
