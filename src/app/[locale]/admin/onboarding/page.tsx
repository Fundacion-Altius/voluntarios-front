...
export default function AdminOnboardingPage() {
  ...
  const handleDelete = async (id: string) => {
    if (!authToken || !confirm(t('eliminarTarea'))) return;
    try {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => { resetForm(); setNewTask(true); }}><Plus className="mr-1 size-4" /> {t('nuevaTarea')}</Button>
      </div>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="space-y-3">
              <Input placeholder={tc('titulo')} value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} />
              <Textarea placeholder={tc('descripcion')} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={2} />
              <Input type="number" placeholder={tc('orden')} value={form.order} onChange={(e) => setForm({ ...form, order: parseInt(e.target.value) || 1 })} />
              <div className="flex justify-end gap-2">
                <Button variant="outline" size="sm" onClick={resetForm}><X className="mr-1 size-4" /> {tc('cancelar')}</Button>
                <Button size="sm" onClick={handleSave}><Save className="mr-1 size-4" /> {tc('guardar')}</Button>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      {tasks.length === 0 ? (
        <p className="text-muted-foreground">{t('noHayTareas')}</p>
      ) : (
