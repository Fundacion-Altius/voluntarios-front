...
function CrearPasswordContent() {
  ...
  const t = useTranslations('crearPassword');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const result = await apiClient(apiUrl('/api/candidates/set-password'), {
        method: 'POST',
        body: JSON.stringify({ token, password }),
      });
      if (!result.success) {
        setError(result.error || t('errorEstablecer'));
        return;
      }
      setSuccess(true);
    } catch (err: any) {
      ...
      <main className="flex min-h-screen flex-col items-center justify-center p-4">
        ...
        <Card className="w-full max-w-md">
          <CardHeader className="text-center">
            <CardTitle>{t('contrasenaEstablecida')}</CardTitle>
            <CardDescription>{t('contrasenaEstablecidaDesc')}</CardDescription>
          </CardHeader>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" className="w-full" disabled={submitting}>
              {submitting ? t('estableciendo') : t('establecerContrasena')}
            </Button>
          </form>
        </CardContent>
      </Card>
